...
def verify_contract_manual():
    """Verify the RightsVault contract on BaseScan using manual verification"""
    ...
    try:
        ...
        if result.get("status") == "1":
            ...
            return verify_contract_alternative()
            
    except Exception as e:
        print(f"❌ Error submitting verification for {CONTRACT_NAME}: {str(e)}")
        return False

def verify_contract_alternative():
    """Try an alternative verification method"""
    print("Using alternative verification method...")
    ...
    # Prepare verification data with different parameters
    verification_data = {
        "apikey": BASESCAN_API_KEY,
        "module": "contract",
        "action": "verifysourcecode",
        ...
        "runs": 0,
        "constructorArguements": "",  # No constructor arguments
        "evmversion": "paris",  # Different EVM version
        "licenseType": 3  # MIT License
    }
    
    # Send verification request
    try:
        print("Sending alternative verification request...")
        response = requests.post(BASESCAN_API_URL, data=verification_data)
        response.raise_for_status()
...
if __name__ == "__main__":
    ...
    success = verify_contract_manual()
    if not success:

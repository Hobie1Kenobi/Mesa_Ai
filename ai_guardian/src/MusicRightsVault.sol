 ...
 * @dev Extension of RightsVault for music-specific rights management
 */
contract MusicRightsVault is RightsVault {
    // Struct for music-specific metadata
    struct MusicMetadata {
        string mesaTrackId;         // MESA unique track identifier
        string title;               // Song title
        string artist;              // Artist name
        uint256 releaseYear;        // Release year
        string[] rightsTypes;       // Types of rights (e.g., "mechanical", "performance")
    }

    // Mapping from rights ID to music metadata
    mapping(bytes32 => MusicMetadata) public musicMetadata;
    
    // Events
    event MusicRightsRegistered(
    ...
    /**
     * @dev Register music rights with metadata
     * @param rightsId Unique identifier for the rights
     * @param encryptedData Encrypted rights data
     * @param dataHash Hash of the original data
     * @param metadata Music-specific metadata
     */
    function registerMusicRights(
        bytes32 rightsId,
        bytes32 encryptedData,
        bytes32 dataHash,
        MusicMetadata memory metadata
    ) external {
        // Register base rights data using parent contract function
        super.registerRights(rightsId, encryptedData, dataHash);
        
        // Store music-specific metadata
        musicMetadata[rightsId] = metadata;
        
        emit MusicRightsRegistered(
            rightsId,
            metadata.mesaTrackId,
            metadata.title,
            metadata.artist
        );
    }
    ...
    /**
     * @dev Get music metadata for specific rights
     * @param rightsId Rights identifier
     * @return MusicMetadata struct containing the music information
     */
    function getMusicMetadata(bytes32 rightsId) external view returns (MusicMetadata memory) {
        return musicMetadata[rightsId];
    }

    /**
     * @dev Verify if rights exist for a specific MESA track ID
     * @param mesaTrackId MESA track identifier to check
     * @return bool indicating if rights exist
     */
    function hasRightsForMesaTrackId(string memory mesaTrackId) external view returns (bool) {
        bytes32[] memory userRights = getOwnerRights(msg.sender);
        for (uint i = 0; i < userRights.length; i++) {
            if (keccak256(bytes(musicMetadata[userRights[i]].mesaTrackId)) == 
                keccak256(bytes(mesaTrackId))) {
                return true;
            }
        }
        return false;
